        ...
        stage("Build") {
            steps {
                script {
                    ...
                    anakondaImage = docker.build(
                        "192.168.40.235/anakonda:jenkins-pipeline-$BUILD_ID",
                        "--build-arg GIT_BRANCH=${gitBranch} --build-arg GIT_COMMIT=${gitCommit} --build-arg GIT_TAG=${gitTag} --build-arg BUILD_TAG=${BUILD_TAG} --build-arg BUILD_ID=${BUILD_ID} ."
                        )
